...
class DataGatherer:
    ...
    def getBestContours(self, tolerance):
        ...
        for i in range(0, len(data) - 1):
            distances.append(math.dist([data[i][0]], [data[i + 1][0]]))
    ...
    def getExtremes(self):
        ...
        data = sorted(newData, key = lambda x2: x2[0])
        c = data[0][2]
    ...
    def getAngle(self, averageCenter, center_x, height):
        angle = math.degrees(math.atan2(self.get_x_offset(averageCenter[0], center_x),  height))
        if angle > 180:
            return 360 - angle
        else:
            return angle
