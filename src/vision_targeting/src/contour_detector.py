...
class ContourDetect:
        ...
        if (is_tracking):
            ...
            if (len(contours) >= 1):
                ...
                if (selected_contour is not None):
                    ...
                    self.add_to_window_t(self.get_center(x, y-h, w, h).x)
                    # Updates feedback for pids
                    self.x_offset = self.get_x_offset(self.average_window_t())
                    
                    # Testing
                    y_temp = self.get_y_offset(self.get_center(x, y-h, w, h).y, height)
                    
                    self.add_to_window(y_temp)
                    self.y_offset = self.average_window()
    ...
    def average_window_t(self):
        total = 0.0
        for x in self.window_t:
            total += x
        return total/self.window_size_t
    
    def add_to_window_t(self, val):
        if len(self.window_t) < self.window_size_t:
            self.window_t.append(val)
        else:
            self.window_t.pop(0)
            self.window_t.append(val)
