...
function updateDefaultAngular(x){
  ...
  //1
  if (autonData.paths[x].max_angular_speed == null){
    document.getElementById("AngularGoalDataMaxSpeedInput" + x).placeholder = "Select Value";
  }
  else{
    document.getElementById("AngularGoalDataMaxSpeedInput" + x).defaultValue = autonData.paths[x].max_angular_speed;
  }

  //2
  if (autonData.paths[x].min_angular_speed == null){
    document.getElementById("AngularGoalDataMinSpeedInput" + x).placeholder = "Select Value";
  }
  else{
    document.getElementById("AngularGoalDataMinSpeedInput" + x).defaultValue = autonData.paths[x].min_angular_speed;
  }

  //3
  if (autonData.paths[x].max_angular_acceleration == null){
    document.getElementById("AngularGoalDataMaxAccelInput" + x).placeholder = "Select Value";
  }
  else{
    document.getElementById("AngularGoalDataMaxAccelInput" + x).defaultValue = autonData.paths[x].max_angular_acceleration;
  }

  //4
  if (autonData.paths[x].angular_tolerance_outer == null){
    document.getElementById("AngularGoalDataTolOuterInput" + x).placeholder = "Select Value";
  }
  else{
    document.getElementById("AngularGoalDataTolOuterInput" + x).defaultValue = autonData.paths[x].angular_tolerance_outer;
  }

  //5
  if (autonData.paths[x].angular_tolerance_inner == null){
    document.getElementById("AngularGoalDataTolInnerInput" + x).placeholder = "Select Value";
  }
  else{
    document.getElementById("AngularGoalDataTolInnerInput" + x).defaultValue = autonData.paths[x].angular_tolerance_inner;
  }

  //6
  document.getElementById("AngularGoalDataIgnoreAngleInput" + x).checked = autonData.paths[x].ignore_angular_tolerance;
}
...
function GoalAngularInput(x){
  ...
  autonData.paths[x].ignore_angular_tolerance = inputValIgnoreAng
