...
function updateDefaultLinear(x){
  ...
  //1
  if (autonData.paths[x].max_linear_speed == null){
    document.getElementById("LinearGoalDataMaxSpeedInput" + x).placeholder = "Select Value";
  }
  else{
    document.getElementById("LinearGoalDataMaxSpeedInput" + x).defaultValue = autonData.paths[x].max_linear_speed;
  }

  //2
  if (autonData.paths[x].min_linear_speed == null){
    document.getElementById("LinearGoalDataMinSpeedInput" + x).placeholder = "Select Value";
  }
  else{
    document.getElementById("LinearGoalDataMinSpeedInput" + x).defaultValue = autonData.paths[x].min_linear_speed;
  }

  //3
  if (autonData.paths[x].max_linear_acceleration == null){
    document.getElementById("LinearGoalDataMaxAccelInput" + x).placeholder = "Select Value";
  }
  else{
    document.getElementById("LinearGoalDataMaxAccelInput" + x).defaultValue = autonData.paths[x].max_linear_acceleration;
  }

  //4
  if (autonData.paths[x].linear_tolerance_outer == null){
    document.getElementById("LinearGoalDataTolOuterInput" + x).placeholder = "Select Value";
  }
  else{
    document.getElementById("LinearGoalDataTolOuterInput" + x).defaultValue = autonData.paths[x].linear_tolerance_outer;
  }

  //5
  if (autonData.paths[x].linear_tolerance_inner == null){
    document.getElementById("LinearGoalDataTolInnerInput" + x).placeholder = "Select Value";
  }
  else{
    document.getElementById("LinearGoalDataTolInnerInput" + x).defaultValue = autonData.paths[x].linear_tolerance_inner;
  }

  //6
  document.getElementById("LinearGoalDataForMoveOnlyInput" + x).checked = autonData.paths[x].forward_movement_only
...
function GoalLinearInput(x){
  ...
  autonData.paths[x].forward_movement_only = inputValForMoveOnly
