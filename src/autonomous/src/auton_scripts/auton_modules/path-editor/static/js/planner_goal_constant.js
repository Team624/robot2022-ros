...
function updateDefaultConstants(x){
  ...
  //1
  if (autonData.paths[x].constants_kP == null){
    document.getElementById("ConstantsGoalDataKpInput" + x).placeholder = "Select Value";
  }
  else{
    document.getElementById("ConstantsGoalDataKpInput" + x).defaultValue = autonData.paths[x].constants_kP;
  }

  //2
  if (autonData.paths[x].constants_kA == null){
    document.getElementById("ConstantsGoalDataKaInput" + x).placeholder = "Select Value";
  }
  else{
    document.getElementById("ConstantsGoalDataKaInput" + x).defaultValue = autonData.paths[x].constants_kA;
  }

  //3
  if (autonData.paths[x].constants_kB == null){
    document.getElementById("ConstantsGoalDataKbInput" + x).placeholder = "Select Value";
  }
  else{
    document.getElementById("ConstantsGoalDataKbInput" + x).defaultValue = autonData.paths[x].constants_kB;
  }
}
